...
import { attributesToString, contentsToString, deepFlatten, isVoidElement } from "./utils";

type Children = number | string | Promise<string> | boolean | null | undefined | Children[];
...
async function createElement(
	...
	return (
		...
		contentsToString(
			children,
			"safe" in attributes && typeof attributes.safe === "boolean" && attributes.safe,
		) +
		"</" +
		name +
		">"
...
function compile<P extends { [K in keyof P]: K extends "children" ? Children : string }>(
...
export { Children, PropsWithChildren, Component, AsyncComponent, SyncComponent };

export { Suspense } from "./suspense";
export { ErrorBoundary } from "./error";
export { renderToStream, renderToStreamResponse, renderToString, renderToStringResponse } from "./render";
