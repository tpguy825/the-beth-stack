...
export function renderToString<T extends () => JSX.Element>(lazyHtml: T): JSX.Element {
	BETH_GLOBAL_RENDER_CACHE.reset();
	const resultPromise = lazyHtml();
	return resultPromise;
}

export async function renderToStringResponse<T extends () => JSX.Element>(lazyHtml: T): Promise<Response> {
...
export function renderToStreamResponse<T extends () => JSX.Element>(lazyHtml: T): Response {
...
export function renderToStream<T extends () => JSX.Element>(lazyHtml: T): ReadableStream<string> {
	BETH_GLOBAL_RENDER_CACHE.reset();
