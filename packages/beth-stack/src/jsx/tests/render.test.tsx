...
import "../register";
import { renderToStream, renderToStreamResponse, renderToString, renderToStringResponse } from "../render";
...
test("multiple children", async () => {
	...
	expect(html).toBe(`<div><div>loaded in: 20ms</div><div>loaded in: 30ms</div><div>loaded in: 10ms</div></div>`);
});
...
test("no render func, doesn't resolve immediately - multiple children", async () => {
	...
	expect(html).toBe(`<div>loaded in: 50ms</div><div>loaded in: 40ms</div><div>loaded in: 10ms</div>`);
});
...
test("renderToStream, doesn't resolve immediately", async () => {
	...
	for await (const chunk of stream) {
		expect(chunk.replace(/[\s\n\t\r]+/g, "")).toBe(expectedChunks[index]!.replace(/[\s\n\t\r]+/g, ""));
		index++;
	}
