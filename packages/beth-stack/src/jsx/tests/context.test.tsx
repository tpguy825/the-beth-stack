...
test("provides and consumes context", async () => {
	const html = await (<MyContext.Provider value={42}>{() => <ConsumerComponent />}</MyContext.Provider>);
...
test("provides and consumes nested context", async () => {
	const html = await (
		<MyContext.Provider value={42}>
			{() => <NestedContext.Provider value="nested">{() => <DualConsumer />}</NestedContext.Provider>}
		</MyContext.Provider>
	);
...
test("outer context is available to inner consumer, but not vice versa", async () => {
	const html1 = await (<MyContext.Provider value={42}>{() => <InnerConsumer />}</MyContext.Provider>);

	expect(html1).toBe(`<div></div>`);

	const html2 = await (
		<NestedContext.Provider value="nested">{() => <OuterConsumer />}</NestedContext.Provider>
	);
