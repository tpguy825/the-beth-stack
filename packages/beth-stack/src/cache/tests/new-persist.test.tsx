...
import { persistedCache, revalidateTag, setGlobalPersistCacheConfig } from "../new-persist";
import "../../jsx/register";
...
describe("interval revalidation", () => {
	...
	test("interval with Infinity or 0 is ignored", async () => {
		...
		const htmlInfinityAfterDelay = await renderToString(() => <ComponentInfinity />);
...
describe("pending behavior (swr)", () => {
	test("by default SWR is on", async () => {
		let count = 0;
		let getCount = async () => new Promise((resolve) => setTimeout(() => resolve(++count), 200));
		let cachedGetCount = persistedCache(getCount, getCacheKey(), {
			revalidate: 1,
		});
