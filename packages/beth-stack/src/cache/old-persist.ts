...
export function persistedCache<T extends () => Promise<any>>(callBack: T, key: string, options?: CacheOptions): T {
	const filledOptions = {
	...
	return cache(() => BETH_GLOBAL_PERSISTED_CACHE.getCachedValue(key, filledOptions.persist)) as T;
}
...
export function setGlobalPersistCacheConfig(config: Partial<GlobalCacheConfig>) {
...
export class BethPersistCache {
	...
	public setConfig(config: Partial<GlobalCacheConfig>) {
		...
		this.config.returnStaleWhileRevalidate =
			config.returnStaleWhileRevalidate ?? this.config.returnStaleWhileRevalidate;

		this.config.onRevalidateErrorReturnStale =
			config.onRevalidateErrorReturnStale ?? this.config.onRevalidateErrorReturnStale;
	}
	...
	private getJsonCache(key: string) {
		const result = this.jsonDataCache.query("SELECT value FROM cache WHERE key = ?").get(key) as
			| { value: string }
			| undefined;
		if (!result) {
			throw new CacheNotFound(`No entry found in json cache when one was expected: ${key}`, "json");
		}
	...
	private rerunCallBack(key: string) {
		...
		callBackPromise
			.then((value) => {
				if (this.config.log) console.log(`Callback complete, setting ${location} cache:`, key);
				if (location === "memory") {
					this.inMemoryDataCache.set(key, value);
				} else if (location === "json") {
					this.setJsonCache(key, value);
			...
			.catch((e) => {
				...
				if (this.config.onRevalidateErrorReturnStale) {
					if (this.config.log) console.log(`Returning stale data dispite error:`, key);
					return this.getCachedValue(key, location);
				} else {
					if (this.neverSeeded.has(key)) {
						if (this.config.log)
							console.log("Never seeded revalidation errored, remaining never seeded:", key);
					} else {
	...
	private setInterval(key: string, revalidate: number) {
		...
		const interval = setInterval(() => {
			if (this.config.log) console.log(`Cache Revalidating (on ${revalidate}s interval):`, key);
			this.rerunCallBack(key);
		}, revalidate * 1000);
		if (this.config.log) console.log("Setting Revalidate Interval:", key, revalidate);
	...
	private getMemoryCache(key: string) {
		...
		if (cacheResult) {
			...
			throw new CacheNotFound(`No entry found in memory cache when one was expected: ${key}`, "memory");
		}
	}
	...
	public getCachedValue(key: string, cache: "memory" | "json") {
		...
		try {
			...
			if (neverSeeded) {
				if (this.config.log) console.log("Never Seeded - Rerunning Callback:", key);
